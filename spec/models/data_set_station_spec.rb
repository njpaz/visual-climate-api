...
RSpec.describe DataSetStation, type: :model do
  let(:noaa_sync) do
    NOAASync.new
  end
  ...
  before do
    ...
    dataset_results.each do |result|
      ...
      stub_request(:get, "http://ncdc.noaa.gov/cdo-web/api/v2/stations/?datasetid=#{result}&limit=1000").
  ...
  context 'populate_joins' do
    it 'retrieves the relevant data and creates a record for each in the data_set_location_categories table' do
      ...
      expect(first_set.stations.pluck(:identifier)).to eq(first_results)
      expect(third_set.stations.pluck(:identifier)).to eq(third_results)
    end
  end
end
