...
RSpec.describe RetryPopulateJob, type: :job do
  let(:noaa_sync) do
    NOAASync.new
  end
  ...
  it 'reruns the specified populate method' do
    DataSet.populate
    Location.populate

    RetryPopulateJob.new.perform DataSetLocation.name, 'populate_joins'

    expect(DataSetLocation.count).to eq(107)
